...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def expires(offer):
            return offer.expirationDate != None and offer.status == 'OPEN'

        def is_expired(offer):
            return expires(offer) and timezone.now().date() > offer.expirationDate

        def _get_total_open_price_by_currency(iss, currency):
            offers = orm.Offer.objects.filter(issue=iss, status='OPEN', currency=currency)
            s = Decimal(0)
            for offer in offers:
        ...
        def _get_total_paid_price_by_currency(iss, currency):
            payments = orm.Payment.objects.filter(status__in=['CONFIRMED_IPN', 'CONFIRMED_TRN'],
                                                  offer__issue=iss,
                                                  currency=currency)
            s = Decimal(0)
        ...
        for issue in orm.Issue.objects.all():
            issue.total_paid_offers_usd = _get_total_paid_price_by_currency(issue, 'USD')
            issue.total_paid_offers_btc = _get_total_paid_price_by_currency(issue, 'BTC')
            issue.total_open_offers_usd = _get_total_open_price_by_currency(issue, 'USD')
            issue.total_open_offers_btc = _get_total_open_price_by_currency(issue, 'BTC')
            issue.count_solutions_done = orm.Solution.objects.filter(issue=issue, status='DONE').count()
            issue.count_solutions_in_progress = orm.Solution.objects.filter(issue=issue, status='IN_PROGRESS').count()
            issue.save()
    ...
    models = {
        ...
        u'core.issue': {
            ...
            'count_solutions_done': ('django.db.models.fields.IntegerField', [], {'default': '0'}),
            'count_solutions_in_progress': ('django.db.models.fields.IntegerField', [], {'default': '0'}),
            ...
            'total_open_offers_btc': ('django.db.models.fields.DecimalField', [], {'default': '0', 'max_digits': '16', 'decimal_places': '8'}),
            'total_open_offers_usd': ('django.db.models.fields.DecimalField', [], {'default': '0', 'max_digits': '16', 'decimal_places': '8'}),
            'total_paid_offers_btc': ('django.db.models.fields.DecimalField', [], {'default': '0', 'max_digits': '16', 'decimal_places': '8'}),
            'total_paid_offers_usd': ('django.db.models.fields.DecimalField', [], {'default': '0', 'max_digits': '16', 'decimal_places': '8'}),
